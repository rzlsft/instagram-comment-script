...
for ($i=0; $i <$adet ; $i++) { 
	...
	sleep($delay);
			...
			break;
			case 1:
$sayiDeger = '1234567890';
$sayiRandom = substr(str_shuffle($sayiDeger), 0, 10);
$newText=$text."  #".$sayiRandom;
	$addComment = $instagram->addComment($mediaId,$newText);
	sleep($delay);

			break;

			case 2:
$yaziDeger = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPUQRSTUVWXYZ';
$yaziRandom = substr(str_shuffle($yaziDeger), 0, 10);
$newText=$text."  #".$yaziRandom;
	$addComment = $instagram->addComment($mediaId,$newText);
	sleep($delay);
